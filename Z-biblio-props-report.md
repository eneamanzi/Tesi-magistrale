...
- [Note di utilizzo](#note-di-utilizzo)
- [FASCIA A — RACCOMANDATO / RACCOMANDATO CON CORREZIONI](#fascia-a--raccomandato--raccomandato-con-correzioni)
  - [Capitolo 2 — Background e Stato dell'Arte](#capitolo-2--background-e-stato-dellarte)
    - [\[A-01\] RESTler: Stateful REST API Fuzzing](#a-01-restler-stateful-rest-api-fuzzing)
    - [\[A-02\] Pythia: Grammar-Based Fuzzing of REST APIs with Coverage-guided Feedback](#a-02-pythia-grammar-based-fuzzing-of-rest-apis-with-coverage-guided-feedback)
    - [\[A-03\] HDiff: A Semi-automatic Framework for Discovering Semantic Gap Attack in HTTP Implementations](#a-03-hdiff-a-semi-automatic-framework-for-discovering-semantic-gap-attack-in-http-implementations)
    - [\[A-04\] Enhancing REST API Fuzzing with Access Policy Violation Checks (EvoMaster)](#a-04-enhancing-rest-api-fuzzing-with-access-policy-violation-checks-evomaster)
    - [\[A-05\] Mining REST APIs for Potential Mass Assignment Vulnerabilities (LightMass)](#a-05-mining-rest-apis-for-potential-mass-assignment-vulnerabilities-lightmass)
    - [\[A-06\] WENDIGO: Deep Reinforcement Learning for Denial-of-Service Query Discovery in GraphQL](#a-06-wendigo-deep-reinforcement-learning-for-denial-of-service-query-discovery-in-graphql)
    ...
    - [\[A-08\] An Analysis of How Many Undiscovered Vulnerabilities Remain in Information Systems](#a-08-an-analysis-of-how-many-undiscovered-vulnerabilities-remain-in-information-systems)
    - [\[A-09\] Automated Black-box Testing of Mass Assignment Vulnerabilities in RESTful APIs](#a-09-automated-black-box-testing-of-mass-assignment-vulnerabilities-in-restful-apis)
    - [\[A-10\] Checking Security Properties of Cloud Service REST APIs](#a-10-checking-security-properties-of-cloud-service-rest-apis)
    - [\[A-11\] OWASP API Security Top 10 — Edizione 2023](#a-11-owasp-api-security-top-10--edizione-2023)
    - [\[A-22\] Testing RESTful APIs: A Survey](#a-22-testing-restful-apis-a-survey)
    ...
    - [\[A-26\] NAUTILUS: Automated RESTful API Vulnerability Detection](#a-26-nautilus-automated-restful-api-vulnerability-detection)
  - [Capitolo 3 — Metodologia](#capitolo-3--metodologia)
    ...
    - [\[A-13\] SATORI: Static Test Oracle Generation for REST APIs](#a-13-satori-static-test-oracle-generation-for-rest-apis)
    - [\[A-14\] Vulnerability-oriented Testing for RESTful APIs (VOAPI2)](#a-14-vulnerability-oriented-testing-for-restful-apis-voapi2)
    ...
    - [\[A-17\] The Oracle Problem in Software Testing: A Survey](#a-17-the-oracle-problem-in-software-testing-a-survey)
    - [\[A-24\] Intelligent REST API Data Fuzzing](#a-24-intelligent-rest-api-data-fuzzing)
    - [\[A-25\] AGORA: Automated Generation of Test Oracles for REST APIs](#a-25-agora-automated-generation-of-test-oracles-for-rest-apis)
...
Il report è organizzato in due fasce di priorità. All'interno di ciascuna fascia, le voci sono raggruppate per capitolo della tesi. Per ogni paper sono riportati: i metadati verificati, la motivazione estesa con riferimenti espliciti al contenuto del paper, il punto di inserimento nella tesi con il label LaTeX, e il BibTeX corretto pronto per l'importazione.

Tutte le correzioni alle voci BibTeX sono marcate con `% CORRETTO:` nel commento inline. I campi segnati `% DA VERIFICARE:` richiedono un passaggio manuale prima dell'inserimento. Ogni voce riporta il punto di inserimento al livello di sezione; dove il claim specifico era direttamente identificabile, è indicata anche la frase di destinazione.

**Composizione del report**: 26 voci in Fascia A (raccomandato o raccomandato con correzioni), 11 voci in Fascia B (tenere con riserva). Il riepilogo completo è in fondo al documento.
...
**Rilevanza**
RESTler introduce il primo fuzzer stateful per REST API in grado di inferire automaticamente le dipendenze producer-consumer a partire dalla specifica OpenAPI/Swagger, superando il limite strutturale dei fuzzer black-box stateless che si schiantano contro l'esplosione combinatoria delle sequenze di chiamate. Il paper dimostra empiricamente che un fuzzer ignaro dello stato genera in massima parte richieste HTTP 400/401 perché non soddisfa le precondizioni degli endpoint profondi, confermando che la copertura logica richiede la comprensione delle dipendenze tra operazioni. È il riferimento canonico per il concetto di "combinatorial wall" discusso nel §2.4 della tesi, nonché la baseline rispetto alla quale si posiziona l'approccio contract-driven di APIGuard. Il paper è già citato indirettamente attraverso la `tab:semantic-gap` (Tabella C) nella tesi; questa voce bibliografica ne formalizza l'inserimento nel testo.

**Punto di inserimento**
`\subsec:semantic-gap` (§2.3.1) per la definizione del semantic gap rispetto al testing sintattico; §2.4 per il combinatorial wall e la necessità del fuzzing stateful. Citabile anche in §3.2 (`\subsec:openapi-sot`) come precedente architetturale che usa OpenAPI come SSoT per la generazione di sequenze.
...
@article{atlidakis2020pythia,
  ...
  author={Atlidakis, Vaggelis and Geambasu, Roxana and Godefroid, Patrice and
          Polishchuk, Marina and Ray, Baishakhi},
...
@inproceedings{shen2022hdiff,
  ...
  author={Shen, Kaiwen and Lu, Jianyu and Yang, Yaru and Chen, Jianjun and
          Zhang, Mingming and Duan, Haixin and Zhang, Jia and Zheng, Xiaofeng},
...
@article{sahin2026enhancing,
  title={Enhancing REST API Fuzzing with Access Policy Violation Checks
...
**Rilevanza**
WENDIGO implementa un agente DRL che, conoscendo solo lo schema GraphQL del target, costruisce automaticamente query semanticamente profonde in grado di esaurire le risorse del database con appena due richieste HTTP per ora. Questo risultato demolisce l'assunzione implicita che il rate-limiting volumetrico (misura di richieste per secondo) sia sufficiente a proteggere un endpoint GraphQL: l'unità di misura rilevante non è la quantità di richieste HTTP bensì la complessità computazionale della singola query. Per la tesi, questo paper è la referenza diretta per la sezione sul Resource Accounting Mismatch e per l'argomento che le difese perimetrali WAF non possono ispezionare il costo computazionale dell'AST GraphQL prima di inoltrarla al backend.

**Punto di inserimento**
§2.8 (GraphQL e bypass delle difese WAF), nella discussione del DoS semantico come classe di attacco distinta dal DoS volumetrico. Il paper supporta direttamente la riga della tabella che recita "Batching rate limit bypass" per GraphQL con API Gateway.

```bibtex
@inproceedings{mcfadden2024wendigo,
  title={WENDIGO: Deep Reinforcement Learning for Denial-of-Service Query
         Discovery in {GraphQL}},
  author={McFadden, Shae and Maugeri, Marcello and Hicks, Chris and
          Mavroudis, Vasilios and Pierazzi, Fabio},
...
#### [A-09] Automated Black-box Testing of Mass Assignment Vulnerabilities in RESTful APIs
...
#### [A-10] Checking Security Properties of Cloud Service REST APIs
...
#### [A-11] OWASP API Security Top 10 — Edizione 2023
...
#### [A-22] Testing RESTful APIs: A Survey

**Metadati**
- **Titolo**: Testing RESTful APIs: A Survey
- **Autori**: Amid Golmohammadi, Man Zhang, Andrea Arcuri
- **Sede**: *ACM Transactions on Software Engineering and Methodology*, vol. 33, n. 1, art. 27, pp. 1--41
- **Anno**: 2023
- **DOI**: `10.1145/3617175`
- **Accesso aperto**: https://arxiv.org/pdf/2212.14604
- **Stato**: Rivista peer-reviewed, ACM TOSEM (top-tier per software engineering). Nessuna correzione necessaria. Andrea Arcuri è lo stesso autore di EvoMaster esteso in [A-04].

**Rilevanza**
Survey sistematico della letteratura sul testing delle API REST, condotto su IEEE Xplore, ACM DL, Semantic Scholar e Web of Science. Il paper classifica e analizza l'intera traiettoria evolutiva delle tecniche di testing REST — dal black-box stateless (scanner di specifica) al fuzzing stateful (RESTler [A-01], Pythia [A-02]) al coverage-guided (Schemathesis [A-12]) — e identifica le lacune aperte tra cui la scarsità di approcci orientati specificamente alle vulnerabilità semantiche. Questa tassonomia è la base di riferimento per il §2.4 della tesi: citare il survey permette di posizionare APIGuard rispetto all'intero campo con una singola fonte autorevole invece che elencare ogni strumento singolarmente. Il paper è citato come riferimento di background da WuppieFuzz [B-03], VOAPI2 [A-14] e SATORI [A-13], che sono già in bibliografia.

**Punto di inserimento**
§2.4 (panoramica degli strumenti di testing REST API e loro limiti), come survey di riferimento che sistematizza la letteratura su cui si basa l'intera analisi dello stato dell'arte.

```bibtex
@article{golmohammadi2023survey,
...
#### [A-26] NAUTILUS: Automated RESTful API Vulnerability Detection

**Metadati**
- **Titolo**: NAUTILUS: Automated RESTful API Vulnerability Detection
- **Autori**: Gelei Deng, Zhiyi Zhang, Yuekang Li, Yi Liu, Tianwei Zhang, Yang Liu, Guo Yu, Dongjin Wang
- **Sede**: 32nd USENIX Security Symposium (USENIX Security 2023), Anaheim, CA
- **Anno**: 2023
- **Pagine**: 5593--5609
- **ISBN**: 978-1-939133-37-3
- **Accesso aperto**: https://www.usenix.org/system/files/sec23fall-prepub-592-deng-gelei.pdf
- **Stato**: Pubblicazione peer-reviewed, USENIX Security 2023 (top-tier). Nessuna correzione necessaria. Già citato da VOAPI2 [A-14] che è in bibliografia.

**Rilevanza**
NAUTILUS introduce una strategia di annotazione della specifica per codificare le relazioni tra operazioni API e le strategie di generazione dei parametri, superando il limite dei vulnerability scanner esistenti che non riescono a rispettare la sequenza corretta delle operazioni durante il testing. Sperimentato su sei servizi RESTful in confronto con quattro scanner state-of-the-art, rileva in media il 141% di vulnerabilità in più e copre il 104% di operazioni in più. Applicato a nove servizi reali, trova 23 vulnerabilità zero-day con 12 numeri CVE assegnati, inclusa una remote code execution in Atlassian Confluence. Per la tesi, NAUTILUS è il riferimento accademico più vicino all'obiettivo di APIGuard — rilevamento automatizzato di vulnerabilità in API RESTful — e la sua presenza in bibliografia è necessaria per due ragioni complementari. Prima ragione: NAUTILUS è già citato in VOAPI2 [A-14] che è in bibliografia; non averlo direttamente è una coerenza mancante che un reviewer noterebbe. Seconda ragione: il confronto con NAUTILUS permette di articolare il contributo originale di APIGuard in modo preciso. NAUTILUS richiede annotazioni manuali della specifica per codificare le relazioni tra operazioni; APIGuard le deriva automaticamente dalla specifica OpenAPI come SSoT, senza annotazioni aggiuntive. Questa distinzione — specifica come oracolo formale vs specifica come punto di partenza per annotazioni manuali — è il contributo differenziante di APIGuard rispetto allo stato dell'arte.

**Punto di inserimento**
§2.4 (stato dell'arte degli strumenti di vulnerability detection per REST API) come lo strumento precedente più simile ad APIGuard, e §3.2 o §3.3 per articolare la distinzione architetturale (annotazioni manuali vs derivazione formale dalla specifica).

```bibtex
@inproceedings{deng2023nautilus,
  title={{NAUTILUS}: Automated {RESTful} {API} Vulnerability Detection},
  author={Deng, Gelei and Zhang, Zhiyi and Li, Yuekang and Liu, Yi and
          Zhang, Tianwei and Liu, Yang and Yu, Guo and Wang, Dongjin},
...
#### [A-12] Deriving Semantics-Aware Fuzzers from Web API Schemas (Schemathesis)
...
- **Stato**: Pubblicazione peer-reviewed. **Correzione applicata**: la sede è ICSE-Companion (proceedings supplementari), non la conferenza principale ICSE.
...
@inproceedings{hatfield2022schemathesis,
  ...
  % CORRETTO: era "ICSE" senza specificare "Companion"
  pages={345--346},
  year={2022},
...
#### [A-13] SATORI: Static Test Oracle Generation for REST APIs
...
- **Stato**: Pubblicazione peer-reviewed, ASE 2025. **Correzione applicata**: la versione originale della voce includeva "Ernst, M.D." come autore (errato) e ometteva Gabriele Bavota. La lista corretta è quella riportata sopra.

**Rilevanza**
SATORI (Static API Test ORacle Inference) genera automaticamente oracoli di test per REST API analizzando staticamente la specifica OpenAPI tramite modelli linguistici di grandi dimensioni, senza necessità di eseguire il sistema target. Il paper valuta l'approccio su 17 operazioni da 12 API industriali, raggiungendo un F1-score del 74.3% e superando il precedente stato dell'arte dinamico (AGORA+, 69.3%). Il risultato più utile per la tesi è la dimostrazione di complementarità tra metodi statici e dinamici: SATORI e AGORA+ insieme coprono il 90% degli oracoli nel ground-truth. Questo supporta la scelta architetturale di APIGuard di ancorare la verifica a oracoli derivati formalmente dalla specifica OpenAPI piuttosto che da euristica dinamica, anticipando anche la discussione sul limite dell'oracle problem nel §6.
...
#### [A-14] Vulnerability-oriented Testing for RESTful APIs (VOAPI2)
...
- **Stato**: Pubblicazione peer-reviewed, USENIX Security (top-tier). **Correzione critica applicata**: la fonte originale attribuiva a questo URL il titolo inventato "Understanding OpenAPI Parsing and Syntax Rules for Fuzzing Automation". Il titolo reale è quello riportato sopra.
...
**Punto di inserimento**
§3.2 (`\subsec:openapi-sot`), come precedente che usa OpenAPI per orientare il testing verso vulnerabilità specifiche, in linea con l'approccio di APIGuard ai domini di garanzia.
...
#### [A-17] The Oracle Problem in Software Testing: A Survey
...
#### [A-24] Intelligent REST API Data Fuzzing

**Metadati**
- **Titolo**: Intelligent REST API Data Fuzzing
- **Autori**: Patrice Godefroid, Bo-Yuan Huang, Marina Polishchuk
- **Sede**: Proceedings of the 28th ACM Joint Meeting on European Software Engineering Conference and Symposium on the Foundations of Software Engineering (ESEC/FSE 2020)
- **Anno**: 2020
- **Pagine**: 725--736
- **DOI**: `10.1145/3368089.3409719`
- **Accesso aperto**: https://dl.acm.org/doi/pdf/10.1145/3368089.3409719
- **Stato**: Pubblicazione peer-reviewed, ESEC/FSE 2020 (top-tier). Stesso gruppo di ricerca di RESTler [A-01] e Checking Security Properties [A-10]. Nessuna correzione necessaria.

**Rilevanza**
Il paper studia come generare intelligentemente i payload di dati inseriti nelle richieste REST per trovare bug nei servizi cloud, usando regole strutturali di fuzzing dello schema OpenAPI (Drop, Change, Duplicate, Expand) e tecniche di apprendimento on-the-fly dai valori nelle risposte precedenti. Sperimentato su Azure DNS e Azure Networking in produzione, trova bug reali con 202 tipi di errore distinti. Per la tesi, questo paper completa la trilogia Microsoft Research e copre il gap specifico della **generazione automatica dei valori dei parametri a partire dagli schemi OpenAPI** (§3.2, §4): RESTler [A-01] risolve le dipendenze tra operazioni, Checking Security Properties [A-10] introduce i property checker, Intelligent Data Fuzzing introduce la generazione intelligente dei dati — le tre componenti che APIGuard integra nel suo approccio contract-driven. Citare tutti e tre della stessa linea conferisce coerenza alla trattazione dello stato dell'arte.

**Punto di inserimento**
§3.2 (`\subsec:openapi-sot`), nella discussione su come la specifica OpenAPI viene usata per derivare i valori dei parametri di test. Citabile anche in §4 nell'implementazione della fase di istanziazione dei parametri.
...
#### [A-25] AGORA: Automated Generation of Test Oracles for REST APIs

**Metadati**
- **Titolo**: AGORA: Automated Generation of Test Oracles for REST APIs
- **Autori**: Juan C. Alonso, Sergio Segura, Antonio Ruiz-Cortés
- **Sede**: Proceedings of the 32nd ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA 2023), Seattle
- **Anno**: 2023
- **Pagine**: 1018--1030
- **DOI**: `10.1145/3597926.3598114`
- **Accesso aperto**: https://personales.us.es/sergiosegura/files/papers/alonso23-issta.pdf
- **Stato**: Pubblicazione peer-reviewed, ISSTA 2023 (top-tier per software testing). Stesso gruppo di ricerca di SATORI [A-13] (Università di Siviglia). Nessuna correzione necessaria.

**Rilevanza**
AGORA genera automaticamente oracoli di test per REST API rilevando invarianti sulle risposte in modalità black-box e dinamica: analizza le risposte delle API su operazioni reali, rileva proprietà che dovrebbero sempre valere (invarianti), e le usa come oracoli per test futuri. Valutato su API con milioni di utenti (GitHub, Marvel, YouTube, Amadeus, OMDb), trova 11 bug reali. AGORA è il predecessore diretto di SATORI [A-13]: SATORI migliora AGORA sostituendo l'approccio dinamico con l'inferenza statica dalla specifica OpenAPI, e lo supera (F1 74.3% vs 69.3%). La presenza di SATORI in bibliografia senza AGORA lascia incompleto il percorso evolutivo della linea di ricerca dell'oracolo automatizzato per REST API, che è centrale in §3.3. Un reviewer che conosce il campo noterà l'assenza. Citare entrambi permette di descrivere l'evoluzione: AGORA introduce il concetto di oracle automatizzato via invarianti, SATORI lo porta al livello statico superando le limitazioni dinamiche.

**Punto di inserimento**
§3.3 (oracolo formale basato su OpenAPI), come predecessore di SATORI [A-13] e come riferimento per l'approccio dinamico all'oracle generation. La struttura consigliata è: "AGORA \cite{alonso2023agora} introduce la generazione automatica di oracoli via rilevamento di invarianti sulle risposte; SATORI \cite{alonso2025satori} porta questo approccio al livello statico, superandone le limitazioni di copertura."

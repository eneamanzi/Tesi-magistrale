% !TEX root = ../main.tex
% AMBIENTI BEAMER CUSTOM DEFINITI IN QUESTO FILE:
%
% \begin{chapter}[image]{color}{title}
%   Slide divisore capitolo con sfondo colorato.
%   image  (facoltativo) — path asset per sfondo, default assets/background_negative
%   color  — colore tema: maincolor, stataledarkgreen, statalegreen,
%             statalelightgreen, statalered, stataleyellow, statalelilla
%   title  — testo del titolo capitolo
%   Nota: aggiungere opzione [fragile] se il contenuto contiene verbatim.
%   Esempio: \begin{chapter}[assets/background_negative]{maincolor}{Background}
%
% \begin{sidepic}{image}{title}
%   Frame con immagine fissa sul lato destro e contenuto a sinistra.
%   image — path asset (senza estensione se supportata dal tema)
%   title — titolo del frame
%   Esempio: \begin{sidepic}{assets/logo_RGB}{Titolo}\end{sidepic}
...
% NOTA: background_alternative non è incluso negli asset di default.
% Sostituire con un asset esistente, es. assets/background o assets/background_negative.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The \texttt{aspectratio} argument to the \texttt{beamer} class is overridden by the \_statale theme

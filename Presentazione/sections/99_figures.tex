...
\begin{frame}{Limite strutturale del fuzzing sintattico}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \node[viaB] (b2) at (a2 |- b1) {Payload validi\\dalla OpenAPI Specification};
...
\begin{frame}{Architettura di APIGuard Assurance}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \draw[arr] (probe-start) -- node[pos=0.6, above, align=center, lbl] {probe\\HyperText Transfer Protocol} (probe-end);
      ...
      {Admin Application Programming Interface\\{\footnotesize WHITE\_BOX}};
...
\begin{frame}{Pipeline a sette fasi}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \node[phase-startup, below=3.5ex of start] (p1) {\textbf{Fase~1:} Inizializzazione\\{\footnotesize\texttt{config/loader.py}}};
      ...
      \node[phase-startup, below=of p3]    (p4) {\textbf{Fase~4:} Test Discovery e Directed Acyclic Graph\\{\footnotesize\texttt{registry.py, dag.py}}};

      \node[phase-exec, below=6.5ex of p4] (p5) {\textbf{Fase~5:} Esecuzione (per test)\\{\footnotesize\texttt{engine.py}}};
      \node[phase-exec, below=of p5]   (p6) {\textbf{Fase~6:} Teardown Last In, First Out\\{\footnotesize\texttt{core/context.py}}};
...
\begin{frame}{Topologia del Directed Acyclic Graph}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
...
\begin{frame}{Gerarchia delle eccezioni}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
        node distance=0.8ex and 2.5em,
        base/.style={draw, rounded corners=1.5pt, inner sep=5pt, text width=17.5em, align=left, font=\small},
        root/.style={draw=gray!50, fill=gray!10, rounded corners=2pt, inner sep=6pt, align=center, minimum width=7em},
...
\begin{frame}{Topologia del testbed}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \node[infra, below=7ex of host] (kong) {\textbf{Kong~3.9 DB-less}\\{\footnotesize :8000 (HyperText Transfer Protocol proxy)}\\{\footnotesize :8443 (Hypertext Transfer Protocol Secure proxy)}\\{\footnotesize :8001 (Admin Application Programming Interface)}};
...
\begin{frame}{Verso una piattaforma modulare}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \textbf{Plugin REST (OpenAPI Specification)} \ {\footnotesize\itshape (attivo)}\\[1.5ex]
...
\begin{frame}{Sanitizzazione a imbuto dell'evidenza}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \node[stage, below=11ex of s1] (s2) {Stadio~2: JSON Web Token heuristic\\{\scriptsize stringa con 2 punti + base64}};
...
\begin{frame}{Error isolation in Fase~5}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tikzpicture}[
      ...
      \draw[msg] ([yshift=-12ex]SC.south) -- ([yshift=-12ex]TGT.south) node[lbl] {HyperText Transfer Protocol request};
      \draw[msg-return] ([yshift=-16ex]TGT.south) -- ([yshift=-16ex]SC.south) node[lbl] {HyperText Transfer Protocol response};
...
\begin{frame}{Domini di Assurance}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tabularx}{\textwidth}{@{} c l >{\raggedright\arraybackslash}X @{}}
      ...
      \addlinespace
      D1               & Identity \& Authentication        & Meccanismi di riconoscimento del chiamante; robustezza del trasporto delle credenziali; ciclo di vita e revoca dei JSON Web Token. \\
      \addlinespace
      D2               & Authorization                     & Logiche di controllo degli accessi; difetti di segregazione orizzontale e verticale (Role-Based Access Control, Broken Object Level Authorization).                 \\
      \addlinespace
      D3               & Data Integrity                    & Coerenza strutturale dei payload; implementazione delle firme crittografiche (Hash-based Message Authentication Code).                                    \\
      \addlinespace
      ...
      \addlinespace
      D6               & Configuration \& Hardening        & Rafforzamento della configurazione del gateway; policy di routing; header di sicurezza HyperText Transfer Protocol.                            \\
      \addlinespace
      D7               & Business Logic \& Sensitive Flows & Falle semantiche complesse: elusione dei vincoli di processo, Server-Side Request Forgery, race condition su operazioni critiche.              \\
      \bottomrule
    \end{tabularx}
  \end{adjustbox}
\end{frame}
...
\begin{frame}{Box Gradient}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tabularx}{\textwidth}{@{} l l >{\raggedright\arraybackslash}X @{}}
      ...
      \addlinespace
      \texttt{WHITE\_BOX} & Accesso Admin Application Programming Interface del gateway & Configurazione statica verificabile per ispezione diretta tramite piano di controllo del gateway.                              \\
      \bottomrule
    \end{tabularx}
  \end{adjustbox}
\end{frame}
...
\begin{frame}{Analisi statica}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tabularx}{\textwidth}{@{} l l >{\raggedright\arraybackslash}X @{}}
      ...
      \addlinespace
      \texttt{pip-audit} & 0 Common Vulnerabilities and Exposures                      &
...
\begin{frame}{Versioni del testbed}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tabularx}{\textwidth}{@{} l l >{\raggedright\arraybackslash}X @{}}
      ...
      \addlinespace
      Kong                        & 3.9 DB-less           & API Gateway: proxy HyperText Transfer Protocol/Hypertext Transfer Protocol Secure, instradamento verso Forgejo, Admin API su porta 8001 \\
      \addlinespace
      ...
      \addlinespace
      \texttt{testssl.sh}         & 3.2.3                 & Analisi Transport Layer Security black-box (test \texttt{ext.1.5.testssl})                                                 \\
      \addlinespace
      \texttt{sslyze}             & (libreria Python)     & Analisi Transport Layer Security via libreria Python (test \texttt{ext.1.5.sslyze})                                        \\
      \bottomrule
    \end{tabularx}
  \end{adjustbox}
\end{frame}
...
\begin{frame}{Modelli di esposizione gateway}
  \begin{adjustbox}{max width=\textwidth, max height=0.82\textheight, center}
    \begin{tabularx}{\textwidth}{@{}
      ...
      \midrule
      Gateway Application Programming Interface Dedicato \newline {\footnotesize (es.\ Kong, Apigee)}
       & Nord-sud
       & Centralizzato (singolo strato)
       & Admin Application Programming Interface diretta
       & Nativa                                      \\
      \addlinespace
      Kubernetes Ingress / Gateway Application Programming Interface
       & Nord-sud
       & Centralizzato (via K8s API)
       & \texttt{kubectl} / Kubernetes Application Programming Interface
      ...
      Gateway Managed Cloud \newline {\footnotesize (AWS, Azure, GCP)}
       ...
       & Software Development Kit / Application Programming Interface del cloud provider
       & Adapter dedicato                            \\
      \bottomrule
    \end{tabularx}
  \end{adjustbox}

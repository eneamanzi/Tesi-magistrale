...
\begin{table}[ht]
  \centering
  \caption{Matrice delle versioni del testbed e dei componenti di assessment
    al momento dell'audit di Milestone~1 (2026-05-17).}
  ...
  \begin{tabularx}{\textwidth}{l l X}
    ...
    \addlinespace
    \texttt{nuclei-templates}   & 10.4.3                & Template pinned per riproducibilità dei scan                        \\
    \addlinespace
...
Il provisioning è deterministico. Il container \texttt{forgejo-setup} usa l'interfaccia \gls{CLI} di Forgejo con flag \texttt{-{}-}or-true, così da non fallire se gli account esistono già da un avvio precedente. Questa scelta è l'implementazione concreta della proprietà di riproducibilità \hyperref[subsec:reproducibility]{D3.P4}: il testbed raggiunge lo stesso stato iniziale indipendentemente da quante volte il comando \texttt{docker compose up} viene eseguito.
...
\begin{table}[ht]
  \centering
  \caption{Risultati dell'analisi statica sul codebase APIGuard Assurance
    v0.1.0 (90 file sorgente), verifiche~1--5 dell'audit di Milestone~1.}
...
Il dato di \texttt{mypy} merita un approfondimento architetturale, perché trascende la semplice metrica di qualità del codice. La proprietà \hyperref[prop:change-me]{D6.P3} (Sezione~\ref{subsec:dual-hierarchy}% \footnote{La Sezione~\ref{subsec:dual-hierarchy} del Capitolo~\ref{chap:implementation} tratta la dual test hierarchy; \hyperref[prop:change-me]{D6.P3} è la proprietà distinta della dual-layer type safety, il cui locus è in \texttt{pyproject.toml} e \texttt{src/core/models/}.}) formalizza un sistema di type enforcement a due strati indipendenti con copertura complementare.


\subsection{Dual-Layer Type Safety (D6.P3)}

Zero errori su 90 file sorgente in modalità \texttt{strict} è un risultato, non un obiettivo. Il problema che questa proprietà risolve ha una struttura precisa: la verifica statica con mypy garantisce che il contratto tra i moduli, espresso tramite type hints, sia consistente lungo l'intera catena di dipendenze. Nessun punto di chiamata riceve un tipo incompatibile con quello dichiarato dal callee. Gli errori di interfaccia tra moduli vengono intercettati prima che il codice venga eseguito contro un target reale.
...
I due strati sono indipendenti e non ridondanti: ciascuno intercetta una categoria di errori che l'altro non vede. Il costo mantenuto è prossimo allo zero per chi aggiunge un test: type hints e modelli Pydantic con \texttt{frozen=True} sono già vincolati dalle Hard Rules del progetto. La safety non dipende dalla disciplina individuale di chi contribuisce.
...
Il verdetto complessivo dell'audit, sintetizzato nella Tabella~\ref{tab:verdetto-release}, è: pronto per la difesa di tesi e per il deployment in ambiente chiuso su target con specifica \gls{OpenAPI} documentata. Gli unici due item deferiti, il file \texttt{LICENSE} e il \texttt{CHANGELOG.md}, riguardano esclusivamente la distribuzione pubblica su PyPI e non hanno impatto sul comportamento operativo del tool.

\begin{table}[ht]
  \centering
  \caption{Verdetto di release-readiness dall'audit di Milestone~1.
    Ogni aspetto ha almeno un numero di verifica corrispondente nel
    registro completo delle 73 verifiche.}
...
I due run sono stati eseguiti in sequenza nello stesso giorno contro lo stesso testbed, senza alcun intervento manuale tra l'uno e l'altro. La Tabella~\ref{tab:idempotenza-kpi} riporta i KPI aggregati; la Tabella~\ref{tab:risultati-per-test} dettaglia lo status e il conteggio dei finding per ciascuno dei 18 test attivi.

\begin{table}[ht]
  \centering
  \caption{KPI aggregati di idempotenza su due run indipendenti. Tutti i valori sono identici o differiscono entro il margine di rumore del sistema operativo. Dati dall'audit di Milestone~1, §A.3.}
  \label{tab:idempotenza-kpi}
  \small
  \begin{tabularx}{0.85\textwidth}{l C C C}
    \toprule
    \textbf{KPI}                           & \textbf{Run~1}    & \textbf{Run~2}    & \textbf{$\Delta$} \\
    \midrule
...
\begin{table}[ht]
  \centering
  \caption{Status e finding count per test, identici su entrambi i run. La colonna \textit{Dominio} riflette la classificazione D0--D7 della metodologia; \textit{Tipo} distingue test nativi da test con tool specializzati. Dati dall'audit di Milestone~1, §A.3.}
...
\begin{table}[ht]
  \centering
  \caption{Verifica live del teardown: stato delle risorse transitorie sul target Forgejo prima e dopo ciascun run. Dati dall'audit di Milestone~1, §A.4.}
...
Zero risorse leakate su due run è la dimostrazione empirica che le proprietà \hyperref[subsec:teardown]{D4.P6} (Best-Effort Teardown \gls{LIFO}) e \hyperref[subsec:reproducibility]{D3.P4} (Riproducibilità) si comportano nel modo dichiarato in presenza di stato reale. Non è sufficiente che la logica di teardown sia corretta in teoria: deve produrre uno stato osservabile verificabile. Queste tabelle sono quella verifica.
...
\begin{table}[ht]
  \centering
  \caption{Metriche di sistema sui due run dell'audit di Milestone~1. Setup single-host, Forgejo~14.0.3 + Kong~3.9 DB-less, suite completa di 18 test in esecuzione sequenziale. Dati dall'audit §A.2.}
...
\begin{table}[ht]
  \centering
  \caption{Topologia del \gls{DAG} di Milestone~1. La Phase~B non può avviarsi finché tutti i test di Phase~A non hanno completato. L'in-degree di \texttt{1.1} è~2: è l'unico nodo consumato da due test in Phase~B. Verificata aciclica tramite \texttt{graphlib.TopologicalSorter} (audit §A.5).}
...
Il risultato aggregato è leggibile in termini precisi. Su un target reale con una specifica \gls{OpenAPI} nativa, APIGuard Assurance v0.1.0 ha eseguito 18 test distribuiti su 7 domini di sicurezza, prodotto 98 finding in 9 categorie distinte, restituito risultati byte-identici su due run indipendenti, drenato tutte le risorse transitorie senza residui, e completato l'intero assessment in circa 4 minuti e 50 secondi su hardware non dedicato. Nessuno di questi numeri è stimato: tutti tracciano a un punto di verifica nell'audit.

Ciò che la validazione empirica non può fare è pronunciarsi sui limiti del paradigma che il tool implementa. Un assessment corretto su una specifica errata produce verdetti formalmente validi ma sostanzialmente privi di significato. Un tool che astrae il gateway per garantire portabilità multi-vendor accetta per costruzione di non poter accedere ai controlli vendor-specifici che non abbiano un equivalente generalizzabile. Queste non sono anomalie dell'implementazione: sono tensioni strutturali del contratto che APIGuard Assurance stringe con il proprio dominio applicativo. La loro analisi è l'oggetto del Capitolo~\ref{chap:discussion}.

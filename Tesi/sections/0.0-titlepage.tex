% !TEX root = ../main.tex
% Pagina di copertina — modifica questo file per ogni nuova tesi.

\subject{
	\uppercase{\huge Università degli Studi di Milano} \\
	\uppercase{\large Facoltà di scienze e tecnologie}
	\vspace{1em}\\
	\uppercase{\large Dipartimento di Informatica }\\
	\uppercase{\large ``Giovanni Degli Antoni''}
	\vspace{1em}\\
	\includegraphics[width=7em]{images/unimi.pdf}
	\vspace{1em}\\
	\uppercase{\large Laurea Magistrale in} \\
	\uppercase{\large Sicurezza Informatica } \\
}

% TODO fix title
\title{\LARGE\uppercase{Progettazione e Implementazione di un Tool API-Agnostico per "il Security Assessment"/"la Security Assurance": un Approccio Contract-Driven Basato su Oracoli Formali in Architetture a Microservizi}}
\author{
	\begin{tabular}{@{}r@{}rr@{}}
		\hspace{10em} & Autore:    & Enea Manzi \\
		\hspace{10em} & Matricola: & 65935A
	\end{tabular}
	\\\\\vspace{1em}
	\begin{tabular}{@{}ll@{}r@{}}
		Relatore:    & Prof. Marco Anisetti           & \hspace{10em} \\
		Correlatore: & Prof. Claudio Agostino Ardagna &               \\
	\end{tabular}
}

% \publishers{Università degli Studi di Milano}
% \uppertitleback{\vspace{3cm}I want to thank Scrooge MacDuck for his financial Support}
\date{A.A. 2025-2026}

...
\subsection{Semantic Exit Codes come Canale di Comunicazione con la Pipeline \texorpdfstring{(\hyperref[subsec:exit-codes]{D6.P1})}{(D6.P1)}}\label{subsec:exit-codes}
...
\subsection{Quality Gate a Due Velocità: Fail-Fast e Assessment Completo \texorpdfstring{(\hyperref[subsec:fail-fast]{D4.P8})}{(D4.P8)}}\label{subsec:fail-fast}

Il parametro \texttt{execution.fail\_fast} nel \texttt{config.yaml} introduce una seconda dimensione di controllo sul comportamento della pipeline, distinta dai semantic exit codes ma complementare ad essi. La distinzione è architetturalmente precisa: \hyperref[subsec:exit-codes]{D6.P1} descrive cosa il tool comunica al termine dell'esecuzione; \hyperref[subsec:fail-fast]{D4.P8} descrive se e quando il tool interrompe l'esecuzione prima del completamento naturale.
...
\paragraph{Plugin System per Test di Terze Parti (\hyperref[subsec:discovery-impl]{D2.P1}).}
